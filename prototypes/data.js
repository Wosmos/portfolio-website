// Prototype data — mirrors src/data/siteData.ts. Content only; no React.
// `year` is the first-commit year where known from a local clone; the rest are
// filled at runtime from the GitHub API (created_at) and fall back to "—".
...
export const person = {
  ...
  cv: "./assets/Wasif_Malik_Resume_SoftwareEngineer.pdf",
  logo: "./assets/wosmo.svg",
...
export const featured = ["zcrypt", "learnity", "netlink", "furnizsh"];

// GitHub repo slug → project id, for the runtime created_at fill.
export const repoIndex = Object.fromEntries(
  projects.map((p) => [p.github.split("/").pop().toLowerCase(), p.id])
);
