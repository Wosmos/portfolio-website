// Metadata helpers shared by the pages and the Open Graph cards.
//
// Server side only. The brand marks are repeated here as path data on purpose: the card renderer runs
// during the build, where it can neither read src/assets nor fetch a URL that is not deployed yet, so
// an inline data URI is the only form that always resolves.

import { SITE_URL } from "@/data/portfolio";

export const SITE_NAME = "wosmo";
/** Bare host, for the places that show a domain rather than link one. */
export const SITE_HOST = new URL(SITE_URL).host;
export const OG_SIZE = { width: 1200, height: 630 } as const;
export const OG_CONTENT_TYPE = "image/png";

/** Card palette — the same tokens the site paints with. */
export const OG_COLORS = {
  ground: "#050508",
  sky: "radial-gradient(circle at 18% 16%, #0d2440 0%, #07090f 58%, #050508 100%)",
  ink: "#f2f5ff",
  cyan: "#00e5ff",
  magenta: "#ff2bd6",
  dim: "#9fb0cc",
...
/** Search results cut off around 160 characters, so nothing ships longer than that. */
export function clampDescription(text: string, max = 158): string {
  const one = text.replace(/\s+/g, " ").trim();
  if (one.length <= max) return one;
  const cut = one.slice(0, max - 1);
  const lastSpace = cut.lastIndexOf(" ");   // never end a description mid-word
  return `${(lastSpace > max * 0.6 ? cut.slice(0, lastSpace) : cut).replace(/[\s,;:.\u2014-]+$/, "")}\u2026`;
}

/** Absolute URL for the places that cannot take a site-relative one (JSON-LD, llms.txt, sitemaps). */
export const absoluteUrl = (path: string): string => new URL(path, SITE_URL).toString();

/** A brand mark as something the card renderer can draw: a data URI plus its intrinsic ratio. */
export interface Mark { readonly src: string; readonly ratio: number }

function mark(viewBox: string, width: number, height: number, d: readonly string[]): Mark {
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${viewBox}">${d.map((p) => `<path fill="#fff" d="${p}"/>`).join("")}</svg>`;
  return { src: `data:image/svg+xml;base64,${Buffer.from(svg).toString("base64")}`, ratio: width / height };
}

/** The width that keeps a mark's aspect at the height a card asks for. */
export const markWidth = (m: Mark, height: number): number => Math.round(height * m.ratio);

const W_PATHS: readonly string[] = [
