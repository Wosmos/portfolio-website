...
export function createSystem({ canvas, labelsEl, projects, onSelect, onSunSelect, onFlightEvent, onBeltLevel, reducedMotion = false }) {
  ...
  const bodies = projects.map((p, i) => {
    const cfg = p.planet, size = cfg.size * PLANET_SCALE, r = ORBITS[i];
    ...
    const atmo = new THREE.Mesh(sphereGeo, new THREE.ShaderMaterial({ vertexShader: V_WORLD, fragmentShader: ATMO_FRAG, transparent: true, depthWrite: false, side: THREE.BackSide, blending: THREE.AdditiveBlending,
      uniforms: { uRim: { value: new THREE.Color(cfg.rim) }, uHot: { value: 0 } } }));
    ...
    el.addEventListener("focus", () => (domHot = i));
    el.addEventListener("blur", () => (domHot = domHot === i ? -1 : domHot));
    el.addEventListener("click", () => onSelect?.(p));
    labelsEl.appendChild(el);

    return { p, root, spin, planet, atmo, ring, pick, el, theta0, size, r, omega, hot: 0, mix: 0,
  ...
  const view = { theta: 0.35, vel: 0, phi: POSES[0][0], radius: POSES[0][1], fit: 1, scroll: 0 };
  ...
  function updateBase() {
    ...
    view.phi = a[0] + (b[0] - a[0]) * f;
    view.radius = (a[1] + (b[1] - a[1]) * f) * view.fit;
  ...
  let active = false, dragging = false, dragMoved = false, dragX = 0, dragY = 0, lastDragX = 0;
  let w = 1, h = 1;
  ...
  stage.addEventListener("pointermove", (e) => {
    ...
    if (dragging) {
      ...
      if (Math.abs(e.clientX - lastDragX) + Math.abs(dy) > 3) dragMoved = true;
      view.theta -= dx * 0.0045; view.vel = -dx * 0.0045;
      dragX = e.clientX; dragY = e.clientY;
    }
  });
  stage.addEventListener("pointerleave", () => { ndc.set(-10, -10); rayHot = -1; sunHot = false; });
  canvas.addEventListener("pointerdown", (e) => { if (!active || focusIdx >= 0 || sunFocus || flight.active) return; dragging = true; dragMoved = false; dragX = lastDragX = e.clientX; dragY = e.clientY; view.vel = 0; canvas.classList.add("is-dragging"); });
  ...
  canvas.addEventListener("click", () => {
    ...
    if (rayHot >= 0) onSelect?.(bodies[rayHot].p);
    else if (sunHot) onSunSelect?.();
  });
  ...
  function resize() {
    ...
    camera.aspect = w / h;
    view.fit = Math.max(1, 1.55 / camera.aspect);
  ...
  let warp = 0, warpDir = 1, hyper = 1, tOrbit = 0, flash = 0, charge = 0, tunnel = 0, lurched = false, beltSide = 0, beltLevel = 0;
  ...
  function poseFor(b, outPos, outLook) {
    ...
    outPos.addScaledVector(UP, R * (b.ring ? 1.6 : 1.15));
    right.crossVectors(tmp.clone().negate(), UP).normalize();
    outLook.copy(P).addScaledVector(right, R * 1.35).addScaledVector(UP, -R * 0.12);
  }
  ...
  function frame(now) {
    ...
    const hits = orbiting && active && !dragging ? raycaster.intersectObjects([...pickables, sun], false) : [];
    rayHot = -1; sunHot = false;
    if (hits.length) { if (hits[0].object.userData.sun) sunHot = true; else rayHot = hits[0].object.userData.index; }
    ...
    const pulse = 1 + 0.12 * Math.sin(t * 0.9) + 0.06 * Math.sin(t * 2.3);
    ...
    for (let i = 0; i < bodies.length; i++) {
      ...
      b.spin.rotation.y = t * (0.22 + (i % 3) * 0.07) * (i === focusIdx ? 1.4 : 1);
      ...
      b.el.style.opacity = String((1 - b.mix) * show * (1 - Math.max(warp, tunnel)) * depth * (view.scroll < 0.6 ? 1 : 0));
      b.el.style.pointerEvents = show && b.mix < 0.5 && view.scroll < 0.6 ? "" : "none";
    ...
    if (flight.active) {
      ...
      if (focusIdx >= 0) {
        ...
        camTarget.addScaledVector(right, pointer.x * b.size * 0.25).addScaledVector(UP, pointer.y * b.size * 0.15);
      } else if (sunFocus) {
        poseForSun(camTarget, lookTarget);
      } else {
        camTarget.copy(camBase);
        lookTarget.copy(lookBase);
        if (!dragging) { camTarget.x += pointer.x * 3.6; camTarget.y += pointer.y * 2.2; }
      }
    ...
    vanish.copy(camPos).addScaledVector(travelDir, 400 * warpDir).project(camera);
    ...
    if (streaks.visible) {
      ...
      streakMat.uniforms.uStretch.value = 0.9 * tunnel * tunnel + 0.12 * tunnel;
    ...
    bloom.strength = 0.5 + 0.25 * tunnel;
