    ...
    blocks: [
      {
        ...
        rows: [
          ...
          ["ADMIN_PASSWORD_HASH", "the bcrypt hash — exactly 60 characters beginning `$2b$12$`. anything else is reported as unconfigured rather than as a wrong password. see the trap below."],
      ...
      { k: "warn", t: "**the dollar trap.** next's dotenv expands `$2b` and `$12` in a bcrypt hash as variable references, so a local `.env.local` must escape every dollar — `\\$2b\\$12\\$…`. login now unescapes it again, so either form works in either place; what it cannot rescue is a hash that was *already* expanded before it was stored — copy from `bun run admin:hash`, never from a file that has been through dotenv." },
      { k: "p", t: "to change the password: `bun run admin:hash '<your password>'` prints a bcrypt hash at cost 12. put it in `ADMIN_PASSWORD_HASH` — escaped or raw, both are accepted — and sign in again. on vercel, pipe it from a file (`npx vercel env add ADMIN_PASSWORD_HASH production < hash.txt`) so no shell eats the dollars." },
